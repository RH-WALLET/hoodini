...
- [x] Icons (16/48/128) — **real artwork**, supplied by Rory 2026-08-07 and
      rebuilt as vector at `assets/icon.svg`: the H with a wallet flap on its
      right leg. Vector because a 1240px source downsampled to 16 turns the
      flap's outline into mush; each size is drawn rather than scaled, and 16px
      uses a simplified variant with the outline dropped and the clasp enlarged.
      `scripts/make-icons.mjs` no longer writes placeholders over them — it
      renders the committed source. A test now reads every PNG chunk's CRC,
      because one of the three shipped corrupt on the first pass: valid
      signature, right dimensions, and half an image
