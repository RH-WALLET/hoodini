...
| **GMGN** | `gmgn.ai` | VERIFIED (on-page) | ~10 chains incl. Robinhood. Charges 1%. DOM captured 2026-08-04 → `docs/dom/gmgn.ai.home.json` (home tab; Trenches not yet captured). **Adapter shipped** (D-051). **Best hooks of the three:** ships Sentry attributes naming its React components (`data-sentry-source-file="TokenItem.tsx"`), and each row carries `href="/robinhood/token/0x…"` — chain slug and address in one attribute. Virtualised list, so nodes recycle. Not in the manifest; would need a new host permission. |
...
| **Terminal (ex-Padre)** | `trade.padre.gg` | VERIFIED (on-page) | Multi-chain, acquired by the Pump.fun team. DOM captured 2026-08-04 → `docs/dom/trade.padre.gg.json`. **Chain and address arrive together in the thumbnail URL** — `thumbnails.padre.gg/ROBINHOOD-0x…` vs `SOLANA-…` — so like Axiom it interleaves chains per row, but with an unambiguous marker. MUI + emotion: the `css-*` classes are build-hashed and unusable, but MUI's own globals (`MuiButton-secondary`, `MuiAvatar-img`) are stable. No `data-*` hooks. **Adapter shipped** (D-051). Row is a classless `div` ×24 holding a `MuiButton-secondary` quick-buy labelled `0.15`; the thumbnail `img` sits inside it, so chain and address are one `querySelector` away. Not virtualised. Not in the manifest. |
